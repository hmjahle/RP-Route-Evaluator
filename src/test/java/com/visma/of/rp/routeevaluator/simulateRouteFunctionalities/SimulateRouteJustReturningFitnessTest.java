...
/**
 * Tests if the fitness found in the RouteSimulator is equal to the
 * one calculated based on the RouteSimulator result.
 * This is only to compare intra route fitness'
 */
public class SimulateRouteJustReturningFitnessTest extends JunitTest {
...
//Tests if the fitness found in the RouteSimulator is equal to the one calculated based on the RouteSimulator result.
    ...
    private void Assert(Individual expectedIndividual) {
        double routeSimulatorReturnedFitnessValue = 0;
        double routeSimulatorResultsCheckFitnessValue = 0;
        for (EmployeeWorkShift empId : problemInstance.getEmployeeStructure().getActiveEmployeeWorkShifts()) {
            RouteSimulatorResult result = routeSimulator.simulateRoute(expectedIndividual, empId);
            expectedIndividual.getFitness().updateEmployeeFitness(result);
            routeSimulatorResultsCheckFitnessValue += expectedIndividual.getFitness().getSingleEmployeeTotalFitness(empId);
            routeSimulatorReturnedFitnessValue += routeSimulator.simulateRouteFitness(expectedIndividual, empId);
        }
        Assert.assertEquals(routeSimulatorResultsCheckFitnessValue, routeSimulatorReturnedFitnessValue, 0.1);
    }
